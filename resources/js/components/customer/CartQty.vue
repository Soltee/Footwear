<template>
	<div class="relative flex items-center">
        <li @click="cartModal = !cartModal" class="relative flex items-center cursor-pointer ml-3 hover:text-custom-gray-lighter">
        ...
        <div v-if="cartModal" class="absolute right-0 top-0 mt-8 p-3 w-64 bg-custom-gray-light rounded-lg text-white">
            <a :href="`/cart-details`">Go To Cart</a>
            <div v-if="products.length > 0" v-for="product in products" class="flex flex-row jusify-start">
                <img class="h-10 w-10 rounded-lg object-cover object-center" :src="`/storage/products/${ (product.imageUrl) ? product.imageUrl : product.attributes.imageUrl }`">
