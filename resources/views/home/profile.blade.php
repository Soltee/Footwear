	        		...
	        		<form method="POST" action="{{ route('customer-update', $customer->id) }}" enctype="multipart/form-data">
		        		...
		        		<div class="w-full">
		        			<div class="flex flex-row items-center 	rounded-lg mb-3">
			        			...
			        			<img class="h-24 w-full rounded-lg object-cover object-center" src="{{ url('/storage/' . $customer->avatar) }}">
			        		</div>
		        			<div class="flex flex-row items-center 	rounded-lg mb-3">
			        			<label for="name" class="bg-blue-800 px-2 py-3 rounded-l-lg text-white text-md	">Name</label>
			        			<input type="text" id="name" name="name"  value="{{ $customer->name }}" class="px-2 py-2 rounded-r-lg bg-blue-700 text-white">
			        		</div>
                            ...
                            <input id="email" type="email" class="shadow appearance-none border bg-blue-700 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline{{ $errors->has('email') ? ' border-red-500' : '' }}" name="email" value="{{ old('email') ?? $customer->email }}" required autofocus>
